...
class MyDatabase:
    ...
    def save_into_database(self, p, size, color, x, y, z):
        ...
        sql = "INSERT INTO robot_pendant (point, size_type, color_type, x_robot, y_robot, z_robot) " \
                "VALUES (%s, %s, %s, %s, %s, %s)"
        sql_where = (p, size, color, x, y, z)
        cursor.execute(sql, sql_where)
        cnt.commit()
        cursor.close()
        cnt.close()

    def update_into_database(self, p, size, color, x, y, z):
        cnt = psycopg2.connect(dbname=self.DB_NAME, user=self.DB_USER, password=self.DB_PASS, host=self.DB_HOST)
        cursor = cnt.cursor(cursor_factory=psycopg2.extras.DictCursor)
        sql = "UPDATE robot_pendant SET size_type=%s, color_type=%s, x_robot=%s, y_robot=%s, z_robot=%s where point=%s"
        sql_where = (size, color, x, y, z, p)
        cursor.execute(sql, sql_where)
        cnt.commit()
        cursor.close()
        cnt.close()
